...
Работай только с указанной кодировкой. При чтении файлов явно указывай, в какой кодировке открываешь (например, `Get-Content -Encoding UTF8`). При сохранении тоже указывай нужную кодировку (`Set-Content -Encoding OEM`). Если нужно переключиться (например, прочитать OEM, потом записать UTF-8), сделай это последовательно и явно напиши в ответе, что и зачем конвертировал. Перед выводом файлов проверяй их в целевой кодировке (`cmd /c "chcp 65001 >nul && type файл"`). В общем описании изменений упоминай, с какими кодировками работал и зачем.
