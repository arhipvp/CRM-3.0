...
```powershell
.\tools\insurance_assistant\scripts\start.ps1
```

Он поднимет локальный Qdrant, соберёт интерфейс и откроет `http://127.0.0.1:8765`. По умолчанию эмбеддинги создаёт Polza.ai (`text-embedding-3-large`); ключ хранится только в игнорируемом `tools/insurance_assistant/.env`. Документы, векторный индекс и история чатов лежат в игнорируемой Git-папке `tools/insurance_assistant/data/`. Подробности и ручной запуск: [tools/insurance_assistant/README.md](tools/insurance_assistant/README.md).
