...
- `deals`: сделки, расчёты (модели `Deal`, `Quote`, `InsuranceCompany`, `InsuranceType`, `SalesChannel`; в `Quote` есть флаги «Официальный дилер» и `GAP`), управление стадиями/статусами и Google Drive-метаданными (удаление файлов и пустых папок из таба «Файлы» — мягкое, через перемещение в подпапку `Корзина` внутри папки сделки).
