...
export const mapClient = (raw: Record<string, unknown>): Client => ({
  ...
  referredBy: toNullableString(raw.referred_by ?? raw.referredBy),
  referredByName: toNullableString(raw.referred_by_name ?? raw.referredByName),
  referredByDeleted: Boolean(raw.referred_by_deleted ?? raw.referredByDeleted ?? false),
  deletedAt: toNullableString(raw.deleted_at ?? raw.deletedAt),
  phone: toOptionalString(raw.phone),
  email: toNullableString(raw.email),
  birthDate: toNullableString(raw.birth_date ?? raw.birthDate),
  sex: toOptionalString(raw.sex),
  birthPlace: toOptionalString(raw.birth_place),
  registrationAddress: toOptionalString(raw.registration_address),
...
export const mapPolicy = (raw: Record<string, unknown>): Policy => ({
  ...
  isRenewed: Boolean(raw.is_renewed ?? raw.isRenewed ?? false),
...
export const mapTask = (raw: Record<string, unknown>): Task => {
  ...
  return {
    ...
    checklistCount: toNumberValue(raw.checklist_count, checklistItems.length),
    createdAt: toStringValue(raw.created_at),
