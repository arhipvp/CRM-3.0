...
- `frontend/.env` нужен в первую очередь для отдельного Vite dev-режима. В локальном prod-like docker-сценарии frontend собирается в образ, а API base path задаётся через `docker-compose.yml`. Подробности: [../docs/local-prod-like-stack.md](../docs/local-prod-like-stack.md).
...
Пользовательские горячие клавиши больше не являются поддерживаемым контрактом интерфейса. Основные действия открываются через кнопки, списки и палитру `Команды`.

Техническая инфраструктура для матчинга сочетаний остается в `frontend/src/hotkeys/*` как внутренний utility и не должна использоваться для новых user-facing сценариев без отдельного продуктового решения.

Если требуется вернуть hotkeys в продукт, это нужно оформлять как отдельную задачу с проверкой конфликтов браузера, явным UI-дизайном подсказок и обновлением тестов/документации.
...
- В `docker-compose.yml` локальный prod-like сервис `frontend` собирает production bundle и отдаёт его из nginx-контейнера; внешний reverse proxy находится в корневом `nginx.local.conf`.
