...
- Иконки: только `AppIcon` и типизированное свойство `icon`. Прикладные action-кнопки не содержат собственные `<svg>` или текстовые псевдоиконки; SVG внутри графиков остаются допустимым исключением. Compact-action иконка должна оставаться различимой при 14–16 px: для неё используется `IconButton`, доступное имя и tooltip.
