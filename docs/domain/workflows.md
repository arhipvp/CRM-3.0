...
5. Preview слияния по умолчанию предлагает самое длинное ФИО, оставляет телефон/email целевого клиента основными, но если они пустые, подставляет первый непустой контакт из дублей. Остальные альтернативные контакты источников добавляются в итоговые заметки. Разные форматы одного телефона считаются одним номером.
6. UI: клик по индикатору нормализации показывает подтверждение `было -> станет`.
7. API: `POST /api/v1/clients/<id>/normalize-name/` меняет только `name`, сохраняет audit log и не запускается автоматически при обычном сохранении клиента.
