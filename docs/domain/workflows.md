...
3. UI: создать полис из сделки; при продлении выбрать предыдущий полис в этой же сделке.
4. API: `POST /api/v1/policies/` (подтягивается client/insured_client; `renews_policy` связывает новый полис с предыдущим).
