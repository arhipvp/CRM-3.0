...
- Удаление ведомости атомарно сохраняет внутренний снимок её состава, отвязывает финансовые записи (`statement = null`) и помечает ведомость удалённой. Повторное удаление после восстановления сохраняет новый актуальный состав.
- Восстановление ведомости возвращает только доступные свободные непроведённые записи подходящего типа с действующими связанными объектами. Текущие суммы и даты записей не откатываются; записи других ведомостей не переносятся. Пропуски возвращаются с причинами.
- Ведомости, удалённые до появления снимков состава, восстанавливаются без автоматического возврата записей. Снимки не публикуются в общем API ведомостей; отчёт восстановления не раскрывает сведения о записях, доступ к которым утрачен.
- `Payment` всегда создается через `Policy`; сделка платежа берется из `Payment.policy.deal`.
- Поле `Payment.deal` остается legacy/денормализованным полем для обратной совместимости, но не является источником правды.
- При переносе `Policy` в другую сделку backend синхронно обновляет `Payment.deal` для всех платежей этого полиса.
