...
from apps.finances.models import Payment, FinancialRecord
...
class TestDealSignals:
    ...
    def test_deal_create_logs_audit(self):
        ...
        log = AuditLog.objects.filter(object_type='deal', object_id=str(deal.id)).first()
...
class TestClientSignals:
    ...
    def test_client_create_logs_audit(self):
        ...
        log = AuditLog.objects.filter(object_type='client', object_id=str(client.id)).first()
        assert log is not None
        assert log.action == 'create'
...
class TestTaskSignals:
    ...
    def test_task_create_logs_audit(self):
        ...
        log = AuditLog.objects.filter(object_type='task', object_id=str(task.id)).first()
        assert log is not None
        assert log.action == 'create'
...
class TestDocumentSignals:
    ...
    def test_document_create_logs_audit(self):
        ...
        log = AuditLog.objects.filter(object_type='document', object_id=str(document.id)).first()
        assert log is not None
        assert log.action == 'create'
    ...
    def test_document_soft_delete_logs_audit(self):
        ...
        # Clear previous logs
        AuditLog.objects.filter(object_type='document', object_id=str(document.id)).delete()
...
class TestPaymentSignals:
    ...
    def test_payment_create_logs_audit(self):
        ...
        log = AuditLog.objects.filter(object_type='payment', object_id=str(payment.id)).first()
        assert log is not None
        assert log.action == 'create'
    ...
    def test_payment_update_logs_audit(self):
        ...
        # Clear previous logs
        AuditLog.objects.filter(object_type='payment', object_id=str(payment.id)).delete()
...
class TestFinancialRecordSignals:
    ...
    def test_financial_record_income_create_logs_audit(self):
        ...
        record = FinancialRecord.objects.create(payment=payment, amount=500.00, source="commission")

        log = AuditLog.objects.filter(object_type='financial_record', object_id=str(record.id)).first()
        assert log is not None
        assert log.action == 'create'
    ...
    def test_financial_record_expense_create_logs_audit(self):
        ...
        payment = Payment.objects.create(deal=deal, amount=1000.00)
        record = FinancialRecord.objects.create(payment=payment, amount=-200.00, source="commission")
    ...
    def test_financial_record_update_logs_audit(self):
        ...
        # Clear previous logs
        AuditLog.objects.filter(object_type='financial_record', object_id=str(record.id)).delete()
...
class TestPolicySignals:
    ...
    def test_policy_create_logs_audit(self):
        ...
        log = AuditLog.objects.filter(object_type='policy', object_id=str(policy.id)).first()
        assert log is not None
        assert log.action == 'create'
