...
from apps.deals.models import Deal, Quote, ActivityLog
...
activities = []
action_types = ['created', 'status_changed', 'stage_changed', 'description_updated', 'assigned', 'policy_created', 'quote_added']
for i, deal in enumerate(deals):
    for j in range(2):
        activity = ActivityLog.objects.create(
            ...
            description='Deystvie #{}: {} dlya {}'.format(j+1, action_types[j % len(action_types)], deal.title),
            old_value='',
            new_value=''
        )
...
print("   OK: {} platezhey i {} finansovykh zapisey\n".format(len(payments), len(financial_records)))
...
for i, deal in enumerate(deals):
    for j in range(2):
        note = Note.objects.create(
            deal=deal,
            body='Zametka #{}: Vazhnaya informaciya o {}. Detalnyy tekst zametki.'.format(j+1, deal.title),
...
for i, deal in enumerate(deals):
    for j in range(2):
        ...
        msg = ChatMessage.objects.create(
            ...
            body='Soobshchenie #{} v chate dlya sdelki {}. Tekst soobshcheniya.'.format(j+1, deal.title)
        )
