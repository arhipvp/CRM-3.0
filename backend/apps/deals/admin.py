...
class DealAdmin(SoftDeleteImportExportAdmin):
    ...
    status_badge.short_description = "‘в вгб"

    def mark_as_won(self, request, queryset):
        updated = queryset.update(status="won")
        self.message_user(request, f"{updated} б¤Ґ«®Є ЇҐаҐўҐ¤Ґ­® ў бв вгб 'ўлЁЈа ­ '")

    mark_as_won.short_description = "ЏҐаҐўҐбвЁ ў ўлЁЈа ­­лҐ"

    def mark_as_lost(self, request, queryset):
        updated = queryset.update(status="lost")
        self.message_user(request, f"{updated} б¤Ґ«®Є ЇҐаҐўҐ¤Ґ­® ў бв вгб 'Їа®ЁЈа ­ '")

    mark_as_lost.short_description = "ЏҐаҐўҐбвЁ ў Їа®ЁЈа ­­лҐ"

    def mark_as_on_hold(self, request, queryset):
        updated = queryset.update(status="on_hold")
        self.message_user(request, f"{updated} б¤Ґ«®Є Ї®бв ў«Ґ­® ­  Ї г§г")

    mark_as_on_hold.short_description = "ЏҐаҐўҐбвЁ ­  Ї г§г"
    ...
    def restore_deals(self, request, queryset):
        ...
        for deal in queryset.filter(deleted_at__isnull=False):
            deal.restore()
            restored += 1
        self.message_user(request, f"‚®ббв ­®ў«Ґ­® {restored} б¤Ґ«®Є")

    restore_deals.short_description = "? ‚®ббв ­®ўЁвм ўлЎа ­­лҐ б¤Ґ«ЄЁ"
...
class SalesChannelAdmin(SoftDeleteImportExportAdmin):
    ...
    fieldsets = (
        ("Ћб­®ў­ п Ё­д®а¬ жЁп", {"fields": ("name", "description")}),
        (
            "ЊҐв ¤ ­­лҐ",
            {
                "fields": ("id", "created_at", "updated_at", "deleted_at"),
...
class QuoteAdmin(SoftDeleteImportExportAdmin):
    ...
    fieldsets = (
        ("Ћб­®ў­ п Ё­д®а¬ жЁп", {"fields": ("id", "deal")}),
        ("‘ва е®ў п Ё­д®а¬ жЁп", {"fields": ("insurance_type", "insurance_company")}),
        ("“б«®ўЁп", {"fields": ("sum_insured", "premium", "deductible")}),
        ("ЏаЁ¬Ґз ­Ёп", {"fields": ("comments",)}),
        ("‘в вгб", {"fields": ("deleted_at",)}),
        ("‚аҐ¬п", {"fields": ("created_at", "updated_at"), "classes": ("collapse",)}),
    )
    ...
    def restore_quotes(self, request, queryset):
        ...
        for quote in queryset.filter(deleted_at__isnull=False):
            quote.restore()
            restored += 1
        self.message_user(request, f"‚®ббв ­®ў«Ґ­® {restored} а бзсв®ў")

    restore_quotes.short_description = "? ‚®ббв ­®ўЁвм ўлЎа ­­лҐ а бзсвл"
