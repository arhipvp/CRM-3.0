...
class DealDriveDownloadTests(AuthenticatedAPITestCase):
    def setUp(self):
        ...
        self.seller = User.objects.create_user(  # pragma: allowlist secret
            username="seller-drive-download",
    ...
    def test_downloads_file_for_closed_deal_when_show_closed_flag_is_set(self):
        ...
        with (
            ...
            patch(
                "apps.deals.view_mixins.drive.download_drive_file",
                return_value=b"closed-pdf",
            ) as download_mock,
        ):
            response = self.api_client.post(
                f"/api/v1/deals/{self.deal.id}/drive-files/download/?show_closed=1",
                {"file_ids": ["closed-file"]},
                format="json",
            )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.content, b"closed-pdf")
        self.assertIn("closed.pdf", response["Content-Disposition"])
        tree_mock.assert_called_once_with("deal-folder")
        download_mock.assert_called_once_with("closed-file")
