...
sales_channel
Если в документе указаны фамилии агентов ("Перебоева", "Лежнев", "Музыченко") — это канал продаж
...
Параметры КАСКО
Заполняй deductible, official_dealer и gap только для полисов КАСКО.
Если тип полиса не КАСКО — deductible = 0, official_dealer = "", gap = false.
deductible — франшиза числом без пробелов и валюты. Если франшиза не указана — 0.
...
def _basic_policy_validate(data: dict) -> None:
    ...
    if not isinstance(deductible, (int, float, str)):
        raise PolicyRecognitionError("deductible должен быть числом или строкой")
    official_dealer = policy.get("official_dealer")
    if not isinstance(official_dealer, bool) and official_dealer != "":
        raise PolicyRecognitionError('official_dealer должен быть true, false или ""')
    if not isinstance(policy.get("gap"), bool):
        raise PolicyRecognitionError("gap должен быть true или false")
...
def _normalize_tristate_boolean(value: object) -> bool | str:
    """Вернуть true/false или пустую строку для неизвестного значения."""
    ...
    if isinstance(value, str):
        normalized = _sanitize_text(value).lower()
        if not normalized or normalized in {"не указано", "не указан", "не найдено"}:
            return ""
    return _normalize_boolean(value, default=False)


def _is_casco_type(value: object) -> bool:
    normalized = _sanitize_text(value).lower()
...
def _normalize_policy_payload(data: dict) -> dict:
    ...
    if isinstance(policy, dict):
        ...
        is_casco = _is_casco_type(policy.get("insurance_type"))
        if is_casco:
            policy["deductible"] = _normalize_amount(policy.get("deductible"))
            policy["official_dealer"] = _normalize_tristate_boolean(
                policy.get("official_dealer")
            )
            policy["gap"] = _normalize_boolean(policy.get("gap"), default=False)
        else:
            policy["deductible"] = "0"
...
def _collect_formal_issues(data: dict) -> list[str]:
    ...
    if deductible in ("", None):
        issues.append(
            "policy.deductible должен быть числом; если франшиза не указана — 0."
        )
    elif not isinstance(deductible, (int, float, str)):
        issues.append("policy.deductible должен быть числом или строкой с числом.")
    official_dealer = policy.get("official_dealer")
    if not isinstance(official_dealer, bool) and official_dealer != "":
        issues.append('policy.official_dealer должен быть true, false или "".')
    if not isinstance(policy.get("gap"), bool):
        issues.append("policy.gap должен быть true или false.")
