...
class DocumentResource(resources.ModelResource):
    class Meta:
        model = Document
        fields = ('id', 'title', 'deal', 'owner', 'doc_type', 'file', 'file_size', 'mime_type', 'status', 'checksum', 'created_at', 'updated_at', 'deleted_at')
        export_order = ('id', 'title', 'deal', 'owner', 'doc_type', 'file', 'file_size', 'mime_type', 'status', 'checksum', 'created_at', 'updated_at', 'deleted_at')
...
class DocumentAdmin(ImportExportModelAdmin):
    ...
    list_display = ('title', 'doc_type', 'owner', 'deal', 'file_size_display', 'status_badge', 'created_at')
    search_fields = ('title', 'doc_type', 'owner__username', 'deal__title')
    list_filter = ('doc_type', 'status', 'created_at', 'deleted_at', 'mime_type')
    readonly_fields = ('id', 'created_at', 'updated_at', 'deleted_at', 'checksum', 'file_size_display')
    ordering = ('-created_at',)
    actions = ['restore_documents']

    fieldsets = (
        ('Основная информация', {
            'fields': ('id', 'title', 'deal')
        }),
        ('Тип и статус', {
            'fields': ('doc_type', 'status')
        }),
        ('Файл', {
            'fields': ('file', 'file_size_display', 'mime_type', 'checksum')
        }),
        ('Владелец', {
            'fields': ('owner',)
        }),
