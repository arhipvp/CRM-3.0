...
class PaymentResource(resources.ModelResource):
    class Meta:
        model = Payment
        fields = ('id', 'policy', 'deal', 'amount', 'description', 'status', 'scheduled_date', 'actual_date', 'created_at', 'updated_at', 'deleted_at')
        export_order = ('id', 'policy', 'deal', 'amount', 'description', 'status', 'scheduled_date', 'actual_date', 'created_at', 'updated_at', 'deleted_at')


class FinancialRecordResource(resources.ModelResource):
    class Meta:
        model = FinancialRecord
        fields = ('id', 'payment', 'amount', 'date', 'description', 'source', 'note', 'created_at', 'updated_at', 'deleted_at')
        export_order = ('id', 'payment', 'amount', 'date', 'description', 'source', 'note', 'created_at', 'updated_at', 'deleted_at')
...
class FinancialRecordAdmin(ImportExportModelAdmin):
    ...
    list_display = ('payment', 'amount_display', 'record_type_badge', 'date', 'description_preview', 'source', 'created_at')
    search_fields = ('source', 'description', 'note', 'payment__policy__number')
    list_filter = ('date', 'created_at', 'deleted_at')
    readonly_fields = ('id', 'created_at', 'updated_at', 'deleted_at', 'record_type_badge')
    ...
    def record_type_badge(self, obj):
        ...
        if obj.amount >= 0:
            return format_html('<span style="background-color: #06ffa5; color: white; padding: 3px 8px; border-radius: 3px;">Доход</span>')
        return format_html('<span style="background-color: #ff006e; color: white; padding: 3px 8px; border-radius: 3px;">Расход</span>')
