...
- `finances`: `Payment` ссылается на `Policy` и `Deal`, содержит сумму и даты, а `FinancialRecord` логирует доход/расход; удаление запрещено после оплаты. Ведомость (Statement) считается выплаченной по факту наличия `paid_at` (поле `status` не используется в бизнес-логике, оставлено для совместимости).
  ...
  Для быстрого редактирования состава используются `POST /api/v1/finance_statements/{id}/attach-records/` и `remove-records/` с непустым массивом `record_ids`. Операции атомарно блокируют ведомость и записи, возвращают обновлённую сводку ведомости (`records_count`, `total_amount`) и фактически добавленные или удалённые идентификаторы. Список `GET /api/v1/financial_records/?statement={id}` постраничный, не включает историю проведённых записей и сохраняет сальдо, нужное для процентного расчёта.
...
- Распознавание полисов через OpenRouter выполняется в два AI-прохода: первичное извлечение JSON и самопроверка результата по исходному тексту/визуальным входам с формальными замечаниями CRM. Для PDF-полисов с плохим или табличным текстовым слоем и для изображений JPG/JPEG/PNG используется Vision ИИ. Управление: `POLICY_RECOGNITION_VISION_FALLBACK_ENABLED`, `POLICY_RECOGNITION_PDF_RENDER_DPI`, `POLICY_RECOGNITION_MAX_VISION_PAGES`; последняя настройка ограничивает суммарное число PDF-страниц и изображений в одном запросе.
