...
- `policies`: `Policy` хранит номер, клиента/страхователя, тип, компанию, канал продаж, даты и автомобильные данные; гарантирует уникальность номера и удаляет зависимые платежи.
- `finances`: `Payment` ссылается на `Policy` и `Deal`, содержит сумму и даты, а `FinancialRecord` логирует доход/расход; удаление запрещено после оплаты.
- `tasks`: `Task` отслеживает статусы (todo, in_progress, done и др.), приоритеты, ответственных и чек-листы, связанные со сделками.
- `documents`: `Document` и `KnowledgeDocument` хранят файлы, метаданные и связи с `Deal`, `User` и Google Drive.
...
- `Policy` имеет индексы по номеру, сделке, страховой и клиентам, автоматически заполняет клиента по `deal`.
- `Payment` иногда включает `deal` как денормализованное поле, перед связью `financial_records` контролирует удаление.
- `Document` хранит файл, тип, чек-сумму и статус (draft/pending/completed/error); `KnowledgeDocument` — библиотечные файлы с Drive-ссылками.
